 ...
 * Retries on transport errors when `opts.retries` is set.
 */
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        try {
            ...
            if (attempt < maxAttempts - 1) {
                await new Promise((resolve) => setTimeout(resolve, opts.retryDelay || 250));
            }
        }
    }
    throw lastError;
...
const buildTask = (num, value) => {
    ...
    if (value.retryDelay) {
        options.retryDelay = value.retryDelay;
    }
    return options;
};
...
/**
 * Resolve a JSON Pointer `$ref` (e.g. `#/components/schemas/Pet`) inside the document.
 */
...
/**
 * If the value is a `$ref`, resolve it inside the document.
 */
const maybeRef = (doc, value) => {
 ...
 * OpenAPI 3 (`servers`, `content`, `examples`, `$ref` resolution) —
 * examples are required to work.
 */
export const buildOpenApiTasks = (doc) => {
    ...
    for (const uriPath of Object.keys(doc.paths)) {
        const pathItem = maybeRef(doc, doc.paths[uriPath]);
        if (!pathItem || typeof pathItem !== 'object') {
            continue;
        }
        const pathParams = (pathItem.parameters || [])
            .map((p) => maybeRef(doc, p))
            .filter((p) => p && p.in);
        for (const method of Object.keys(pathItem)) {
            if (['parameters', 'servers', 'summary', 'description', '$ref'].includes(method)) {
                continue;
            }
            const operation = maybeRef(doc, pathItem[method]);
            ...
            for (const status of Object.keys(operation.responses)) {
                const response = maybeRef(doc, operation.responses[status]);
                if (!response) {
                    continue;
                }
                const task = {
                ...
                params.filter((p) => p.in === 'path').forEach((p) => {
                    const example = isOas3 ? exampleFor(doc, p) : p.example;
                ...
                if (isOas3) {
                    ...
                    if (contentType) {
                        const data = exampleFor(doc, response.content[contentType]);
                        task.response.headers = { contenttype: contentType };
                        if (data !== undefined) {
                            task.response.data = data;
                        }
                    }
                    const requestBody = maybeRef(doc, operation.requestBody);
                    if (requestBody && requestBody.content) {
                        const bodyType = mediaType(requestBody.content);
                        if (bodyType) {
                            const bodyExample = exampleFor(doc, requestBody.content[bodyType]);
...
const exampleFor = (doc, schema) => {
    ...
    if (schema && schema.schema) {
        const resolved = maybeRef(doc, schema.schema);
        return exampleFor(doc, resolved);
    }
    return undefined;
};
...
export const setOutputs = (results) => {
    results.forEach((value) => {
        ...
        if (value.result && value.result.error) {
            ...
            value.output.msg.push(value.result.error);
        } else if (value.task.response) {
            value.output.pass = false;
    ...
    groups.forEach((group) => {
        group.tasks.forEach((item) => {
            ...
            if (item.result && item.result.error) {
                ...
                item.output.msg.push(item.result.error);
