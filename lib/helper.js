    ...
    if (value.task.response.data) {
        ...
        if (value.result.headers && value.result.headers['content-type'] === 'application/json') {
            try {
                ...
                console.log(typeof value.task.response.data);
                console.log(value.task.response.data);
        ...
        if (value.task.response.regex) {
            ...
            const result = Joi.validate(resultBody, value.task.response.data);
            if (result.error) {
            ...
            if (typeof resultBody === 'object') {
                if (typeof value.task.response.data === 'string') {
                    try {
                        let requestBody = JSON.parse(value.task.response.data);
                ...
                } else if (value.task.response.data instanceof Buffer) {
